...
const int speedOne = 800;
const int waitTimeOne = 1000;

//position 2 profile
const int positionTwo = -9000; //position 2 in degrees of stepper rotation //negative value for CCW, positive for CW
...
void setup() {
  stepper.setup(CLOSEDLOOP,200); //resets encoder and introduces runmode as CLOSEDLOOP
...
void loop() {
  ...
  if(digitalRead(buttonPin1)== LOW && digitalRead(topSwitchPin)== HIGH)                     
   {
    Serial.println("button1");
    moveTo(positionOne,currentValOne,speedOne,waitTimeOne);
   }
    
  else if(digitalRead(buttonPin2)== LOW && digitalRead(topSwitchPin)== HIGH)
  {
    Serial.println("button1");
...
void homing() { //goes back until it is jamed, then resets position to 0
  ...
  delay(200);
  stepper.setup(CLOSEDLOOP,200); //resets encoders position to 0
  stepper.setCurrent(currentValDefault);

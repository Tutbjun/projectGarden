...
const int liftToURCom = 5;

const int preHomingPos = -100; //the angle at wich the stepper parks before doing the homing operation after a move
const bool runClosedLoop = false; //if true, it current causes issues with jitterring at standstill
...
//position 1 profile
const int positionOne = -2000; //position 1 in degrees of stepper rotation
const int currentValOne = 90;
const int speedOne = 400;
const int waitTimeOne = 1000;

//position 2 profile
const int positionTwo = -7000; //position 2 in degrees of stepper rotation //negative value for CCW, positive for CW
const int currentValTwo = 90;
const int speedTwo = 400;
const int waitTimeTwo = 1000;

//position 2 profile
const int positionThree = -13500; //position 2 in degrees of stepper rotation //negative value for CCW, positive for CW
...
void setup() {
  ...
  pinMode(liftToURCom, OUTPUT);
  homing();

  digitalWrite(liftToURCom , LOW);
}
...
void loop() {
  ...
  else if(digitalRead(buttonPin2)== LOW && digitalRead(buttonPin1)== LOW && digitalRead(topSwitchPin)== HIGH)
  {
...
{
  digitalWrite(liftToURCom , HIGH); //comunicates that it is moving
  ...
  digitalWrite(liftToURCom, LOW); //comunicates that it has finished moving
}
